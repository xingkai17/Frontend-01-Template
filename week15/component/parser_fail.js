...
// in script reseived </sc
function scriptDataEndTagEnd(c) {
  if (c === ' ') {
    return scriptDatLessThanSign
  }
}
...
module.exports.parseHTML = function parseHTML(html) {
  ...
  for (const char of html) {
    state = state(char);
    if (stack[stack.length - 1].tagName === 'script') {
      state = scriptData
    }
  }
